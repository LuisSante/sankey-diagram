<script>
	...
	async function exportToPDF() {
		...
		try {
			// Cargar jsPDF usando un script tag
			if (!window.jsPDF) {
			...
			// Crear un canvas temporal para convertir SVG
			...
			img.onload = function () {
				...
				// Crear PDF usando window.jspdf.jsPDF
				...
				if (imgAspectRatio > pdfAspectRatio) {
					finalWidth = pdfWidth - 20; // margen
					finalHeight = finalWidth / imgAspectRatio;
					x = 10;
					y = (pdfHeight - finalHeight) / 2;
				} else {
					finalHeight = pdfHeight - 20; // margen
...
<div class="sankey-container">
	<div class="controls">
		<button class="export-btn" on:click={exportToPDF}> 📄 Exportar PDF </button>
	</div>
